...
let base_env =
  ("add", Parse.string_to_type
             "x:int{v:True}->y:int{v:True}->int{z:z=x+y}") >:
    (("sub", Parse.string_to_type
               "x:int{v:True}->y:int{v:True}->int{z:z=x-y}") >:
    E_Empty)
...
let implication (x : A.var) (t : A.ty) (c : L.constraint_) :
    L.constraint_ =
  match t with
  | A.T_Refined (bt, v, p) ->
      (x, ty_to_sort bt, L.substitute_pred p v x) ==> c
  | _ -> c
...
let rec substitute_type (t: A.ty) (y: A.var) (z: A.var) : A.ty =
  ...
  | T_Refined (b, v, p) ->
     if String.equal v y
     then t
     else T_Refined (b, v, L.substitute_pred p y z)
  | T_Arrow (v, s, t) ->
     if String.equal v y
  ...
  | T_Refined (b, v1, p1) -> (
    ...
    | T_Refined (b', v2, p2) -> (
      ...
      then raise (Subtyping_error "Refined types have different base types")
        ...
        (v1, L.S_Int, p1) ==> (L.C_Pred (L.substitute_pred p2 v2 v1))
  )
...
let rec check (g : env) (e : A.expr) (ty : A.ty) : L.constraint_ =
  let _ = if !debug then (print "check";
          dbg @@ pp_expr e;
          dbg @@ pp_ty ty) in
  match e with
  | E_Let (x, e1, e2) ->
...
and synth (g : env) (e : A.expr) : L.constraint_ * A.ty =
  let _ = if !debug then (print "synth";
          dbg @@ pp_expr e) in
  match e with
  | E_Const c ->
     let t = A.T_Refined (A.B_Int, "v", L.P_Op (L.O_Eq, L.P_Var "v", L.P_Int c)) in
     (L.C_Pred L.P_True, t)
  | E_Var v -> (
  ...
  | E_App (e, y) ->(
    ...
    | (_, T_Refined _) -> raise (Synthesis_error "Expected exp to synthesize to arrow type")
  )
